...
class LessonTestCase(APITestCase):
    ...
    def test_get_list_lessons(self):
        user = User.objects.create_user(
            email="testuser@example.ru", password="testpassword"
        )
    ...
    def test_lesson_create(self):
        user = User.objects.create_user(
            email="test2user@example.ru", password="test2password"
        )
        self.client.force_authenticate(user=user)
    ...
    def test_lesson_retrieve(self):
        user = User.objects.create_user(
            email="test3user@example.ru", password="test3password"
        )
        self.client.force_authenticate(user=user)
    ...
    def test_lesson_update(self):
        user = User.objects.create_user(
            email="test4user@example.ru", password="test4password"
        )
        self.client.force_authenticate(user=user)
        ...
        response = self.client.put(
            reverse("course:lesson_update", args=[self.lesson.id]), data=data
        )
    ...
    def test_lesson_destroy(self):
        user = User.objects.create_user(
            email="test5user@example.ru", password="test5password"
        )
        self.client.force_authenticate(user=user)
...
class SubscriptionCourseTestCase(APITestCase):
    ...
    def test_subscription_course_create(self):
        user = User.objects.create_user(
            email="test6user@example.ru", password="test6password"
        )
        self.client.force_authenticate(user=user)
        ...
        self.assertEqual(response.status_code, status.HTTP_201_CREATED)
        self.assertEqual(
            response.data["detail"], "Подписка на данный курс выполнена успешно!"
        )
    ...
    def test_subscription_course_destroy(self):
        ...
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(
            response.data["detail"], "Вы успешно отписались от данного курса!"
        )
